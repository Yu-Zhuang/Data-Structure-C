...
*/
#define MAX 7 //資料為 幾乘幾 的矩陣
#define MAX_WEIGHT 7*10 //最大權重為10 再乘以 矩陣行列數
...
bool FIND(int map[][MAX], int start, int end, int *path, int *pathSize,int *pathWeight){
	...
	while(stack->next){
		if(stack->next->val == end){ //找到 end 
			...
			// 如果走完
			if(! stack->next) { for(int i=0;i<minpath_Size;i++) path[i]=min_path[i]; pathSize[0]=minpath_Size; pathWeight[0]=min_weight; return find; }
			stack->next->val *= -1;
		}
		else{ // 未找到 end
			flag = 0; //判斷是否已達終點
